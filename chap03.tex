...
The important point is that we want the result of combining the methods to behave exactly like a normal function or method. The caller should not know and should not have to know that he is calling an object that is somehow special. This allows us to limit the adaptation framework interaction in our system to one place which could be hidden inside a separate module behind an interface, and gives us the possibility to quickly replace it or remove it completely.
...
Note that declaring the \inlinecode{or} method with \inlinecode{MultiFunctionN} argument directly instead of using the \inlinecode{FunctioN} argument (or its alias) and then manually converting it would effectively block the implicit \textit{eta-expansion} of the argument.
...
So far, all mentioned usages of the \inlinecode{or} method were limited to functions with the same signatures. Quite common case, however, might be combining multiple functions with slightly different argument and return value types, typically one being a specialized version of the other. 
...
Let's examine how this can be done on a simplified case with more combinations:
...
The type arguments representing the function arguments are defined as contravariant and the type argument representing the return value of the function is defined as covariant. This leads to \inlinecode{(String) => String} being a subtype of \inlinecode{(String) => Any} and to \inlinecode{(Any) => String} being a subtype of \inlinecode{(String) => String}. So the \inlinecode{or} method is working flawlessly for the \inlinecode{fun5} and \inlinecode{fun6}.

In general, the target of the \inlinecode{or} call (i.e. the \inlinecode{MultiFunctionN} instance created by the implicit conversion) determines the type of the resulting function, and only its subtypes can be passed in as arguments. This can solve majority of the cases where it would be enough to put the more general function on the left side of the \inlinecode{or} method. In the case where we wanted the combined function to have more general signature, we can perform the typecast on the first function passed in:
...
\end{lstlisting}
This, however, lead to problems when being invoked as macro, so this solution was not used in the final version.
...
def count[A](list: List[A], item: A) = (defaultCount[A] _ or customCount[A])(list, item)
	\end{lstlisting}
	This approach was already mentioned in \ref{subsubsec:method_from_combined_func} and is discouraged from as it leads to repeated creation of the \inlinecode{MultiFunctionN} implementation.
...
In this case, we would need to expand the type argument to a type argument of an enclosing scope (following the solutions from section \ref{subsec:generics}) in order to preserve the genericity. Consequentially, we need to receive the implicit arguments from the enclosing scope as well. Ideally, we would like to propagate the implicit argument all the way to the level where the type arguments are set, because that is the first point where an actual implementation can be provided. The most straightforward solution to this is just repeating the implicit argument of the type in each of the enclosing scopes where the type argument still exists. This can be done, again, at the level of methods:
...
If we design our own traits for the use in the application, we can intentionally expose the logic using only functions. In that case, implementing the trait using combined functions does not represent any problem. In case that we need to use an existing trait that contains methods, we need to implement them and delegate the call to the combined function using the techniques described in section \ref{subsubsec:method_from_combined_func}.
...
In this section, we will introduce the rest of the API of the \inlinecode{MultiFunction} type that the library user will be working with. This part of the API is not essential as it is possible to combine functions and use the library without it. It can, however, be used to control the selection functionality and behavior and to examine its results.

\subsection{Combined function configuration}
\label{subsec:api_function_configuration}

When creating a \inlinecode{MultiFunctionN} instance using the implicit conversion and the \inlinecode{or} method, there is no way how to introduce some additional configuration. In compliance with the DSL-like fluent \inlinecode{or} chaining, the configuration was made possible via special methods on the \inlinecode{MultiFunctionN} type that allow chaining as well\footnote{In the \textit{builder}-style pattern, by returning the same type.} and are also meant to be used like operators. The combined function definition including the configuration can be very fluent and natural:
...
The most important part of the configuration API is the \inlinecode{by} method. It allow the user to specify how to create an \textit{input descriptor}, integer that describes the input from the perspective of expected complexity, from the function arguments upon invocation. A function of type \textit{(T1, ..., TN) => Long} is used for that, where \inlinecode{T1, ..., TN} are the arguments of the \inlinecode{MultiFunctionN}.

The rest of the configuration options will be described in later chapters and in the documentation to the framework.

\subsection{Analytics and control access}

There is also a need to monitor and control the \inlinecode{MultiFunctionN} in case the user is aware that he is dealing with a combined function. A simple set of methods on the type allow him to do the following:

\begin{itemize}
	\item Train the combined function on a given set of inputs (by executing all of the functions on each of the inputs and storing the results)
	\item Flush entirely the run history of given function
	\item Set and reset the policy used in the function (will be discussed later in more details)
	\item Retrieve analytics data regarding the function history
\end{itemize}

Analytics data allow the user to observe and analyze the decisions of the combined function. It contains a complete set of records of previous runs, each one holding the identifier of the selected function, the \textit{input descriptor}, run time and the overhead time spent on the selection. The data can either be analyzed directly in the application by processing these records, or can be easily serialized into a CSV file.
...
This approach has one main disadvantage compared to the rest of the API - it breaks the transparency. The user has to know that he is working with the \inlinecode{MultiFunctionN}, not just normal function. His assistance is, however, required by the nature of the problem.
