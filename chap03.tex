...
\section{Measurement storage location}
\label{sec:storing}
...
The first idea might be to store all the measured runtime data inside the function object itself - we have a custom type FunctionAdaptor, that is hidden to the user and that carries the references to function implementation, so it could hold the measurements for them as well.
...
If we abandoned the idea of meaningfulness, we could assign automatically generated identifiers without the user having to specify them. Possible candidates could be sequential numbers or GUIDs. The identifiers would have to be created and matched with the function definitions at compile time, because the functions can be used repeatedly and in different contexts. This could be achieved using the Scala def macros (see \ref{sec:defmacros}). The main disadvantage of this approach is the impossibility of keeping the identifiers synchronized across various compilations and thus reusing the measured data in longer terms.

\subsection{Fully qualified names of the methods combined}
The most reasonable approach is to identify the methods using the same key that is used in the compilation process. It will keep the information value and won't complicate the API for the user. Retrieving the name of the method is, however, not trivial. Considering the following example:
...
Line 5 will print out \textit{Test\$\$anonfun\$main\$1}, a name of a closure class generated to encapsulate lambda expression. The method passed into the \lstinline|printName()| method was eta-expanded and thus replaced by a lambda expression with a single call inside. The lambda expression gets compiled and so at runtime we have absolutely no way of finding out which method is called inside.

There are two basic ways how to handle the problem:
\begin{enumerate}
	\item Use the enclosing closure type names instead
	\item Fetch the actual names at compile time
\end{enumerate}

The first approach leads to unique, but uninformative names, that can get mixed when recompiling the program, so it wouldn't be safe to preserve the measurements. Additionally, if we use the method in more different lambda expressions (or if it gets eta-expanded implicitly multiple times in the code), it will have different identifiers for these cases. This isn't so problematic, it copies the behavior of 
