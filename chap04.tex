...
The most complicated task of the entire chain is to select the most appropriate function to run in given case. The case is described by an input $in$ of the function \inlinecode{f()} and by the evaluation data gathered in previous runs. What we basically need to do is to use the historical data to formulate a prediction of the run times of all the functions, and then try to decide, which one is the most optimistic.
...
\subsection{Input in the selection process}
...
Based on this approach, we will suppose that for each Scala function \(\exists g\) so that \(\forall in\) modeled by $f_1, ..., f_n$ holds \(t = g(f_1, ..., f_n) + \varepsilon\), where \(t\) is the runtime on input \(in\) of \inlinecode{f()}, and $\varepsilon$ is an error represented by a random variable with normal distribution.
...
In some cases, the function run time does not depend on the input, or the relation is not significant enough. For simplification, we suppose that the function complexity is constant in such a case. Using the same formalism as in \ref{subsec:input_in_selection}, for run time $t$ on input $in$ with \textit{input descriptor} $desc$ holds \(t = h(desc) + \varepsilon\) for a $h$ constant, i.e. $\forall x$ $g(x) = c$. Therefore, $t = c + \varepsilon$, where $\varepsilon$ is the error which we suppose has a normal distribution with a zero mean. Under these assumptions, $t$ is a random variable with normal distribution and mean equal to $c$, which is the expected constant run time of the function.

The \textit{mean based} strategies will use the sample data to determine the means of the function time distributions with given certainty and use them as the expected run times in the decision process.
...
\lstset{style=Pseudo}
\begin{lstlisting}
select from f1, f2 with data1, data2
p = tTest(data1, data2)
if (p > 2 * alpha) use secondary selector
if (mean(data1) < mean(data2)) select f1
else select f2
\end{lstlisting}

\begin{algorithmic}[1] % The number tells where the line numbering should start
	\INPUT$f1_,f_2,data_1,data_2$
	\State $equalityRejected \gets twoSidedTTest(data_1, data_2, \alpha)$
	\If{not $equalityRejected$}
	\State use secondary selector
	\EndIf
	\If {$mean(data_1) < mean(data_2)$}
...
\begin{algorithmic}[1] % The number tells where the line numbering should start
	\INPUT$f_1,...,f_n,data_1,...,data_n$
	\For{$i = 1$ to $n$}
		\State $better \gets true$
		\For{$j = 1$ to $n$}
			\State $equalityRejected \gets oneSidedTTest(data_i, data_j, \alpha)$
			\If{(not $equalityRejected$) or $mean(data_i) > mean(data_j)$}
...
\begin{algorithmic}[1] % The number tells where the line numbering should start
	...
	\State $better \gets true$
	\For{$j = 1$ to $n$}
	\State $equalityRejected \gets oneSidedUTest(data_i, data_j, \alpha)$
	\If{(not $equalityRejected$) or $mean(data_i) > mean(data_j)$}
	\State $better \gets false$
	\EndIf
	\EndFor
...
\begin{algorithmic}[1] % The number tells where the line numbering should start
	\INPUT functions $f_1,...,f_n$, historical data vectors $d_1,...,d_n$, input descriptor $x$
	\For{$i=1$ to $n$}
...
The strategy itself is parametrized by an average record count per window $avg$.


\begin{algorithmic}[1] % The number tells where the line numbering should start
	\INPUT 
	\Statex Functions $f_1,...,f_n$
	\Statex Historical data vectors $d_1,...,d_n$ where $d_i = [(x_{i,1}, y_{i,1}),...,(x_{i,l_i}, y_{i,l_i})]$
	\Statex Input descriptor $x$
	\Statex
	\For{$i=1$ to $n$}
	...
	\EndFor
	\State use LinearRegressionStrategy with $f_1,...,f_n, d'_1,...,d'_n, x$
\end{algorithmic}
...
The selection algorithm is exactly the same as in section \ref{subsec:window_bound_regression}, only using 
...
\begin{algorithmic}[1] % The number tells where the line numbering should start
	...
	\State $d'_i \gets [(x_{i,j}, y_{i, j}); \forall j: \abs{x_{i,j} - x} \leq w/2]$
	\EndFor
	\State use TTestStrategy with $f_1,...,f_n, d'_1,...,d'_n, x$
\end{algorithmic}
